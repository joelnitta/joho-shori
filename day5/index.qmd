...
- これから学ぶ関数はもっと大きなデータに使うので、**新しいプロジェクト**を作ります。
...
- でも、データフレームに入っている値の**平均や合計を計算したい場合**は、それでは足りません。
...
- `summarize(データ, 新しい列名 = コマンド)`
  ...
  - 新しい列名の設定は**任意です**。自分にとって分かりやすい名前にしましょう。
...
- `min()`はもっとも小さい値を返す関数です。

## グループ化

- しかし、**全体の**平均や最小値を検出するだけではあまり役に立ちません。

- それよりも知りたいのは**国ごと**の値なのです。

- 何か**毎に**計算するには、**グループ化**を使います。
...
- でも、グループ化しただけでは計算まだ出来ていません。

- グループ化の次に`summarize()`で計算します：
...
- 国毎の一人当たり総生産の平均と標準偏差を計算してください

## パイプライン

- パイプの説明にも出ましたが、パイプとコマンドを組み込むことによって、複雑なデータ処理を簡単に行うことができます。

- 例えば、アジアの国だけに絞ってから、国毎の計算もできます：
...
- グループ化することによってグループ毎の集計ができます
  - `group_by`はグループを定義する
  - `summarize()`はグループ毎の計算を行う

- パイプ（`|>`）で複数のデータ処理作業を繋ぐことによって、複雑なデータ処理パイプラインを組むことができます
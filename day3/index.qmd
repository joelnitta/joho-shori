...
#| eval: false
# Load data
students <- read_csv("Z:/Desktop/students_project/students.csv")
```
...
- つまり、R の作業ディレクトリ（R が今入っているフォルダー）から見たパスです。
...
- 今、Rは`Z:/Desktop/students_project/`に入っていることが分かりました。
...
- なので、ファイルの場所がすぐに分かり、プロジェクトを別の場所に移動しても、コードを変える必要がありません。
